...
type MainView(window : MainWindow) = 
    ...
    let priceFrequency = DispatcherTimer(Interval = TimeSpan.FromSeconds 0.5)
    let priceTicksSource = 
        priceFrequency.Tick |> Observable.choose (fun _ ->   
    ...
    interface IView<MainEvents, MainModel> with
        member this.Events = 
            [
                ...
                priceTicksSource |> Observable.map (fun value -> PriceUpdate value)
